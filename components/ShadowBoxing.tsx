...
export const ShadowBoxing: React.FC<ShadowBoxingProps> = ({ state, setState }) => {
  ...
  const handleCallout = async (fallacyId: string) => {
    ...
    if (lastAdversaryTurnIndex === -1) return;
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-12 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="surface p-8 rounded-2xl border-rose-500/20">
            ...
            <div className="space-y-3">
              {DEBATE_TOPICS.map((topic, i) => (
                <button
                  ...
                  disabled={loading}
                  className="w-full text-left p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-rose-500/40 hover:bg-rose-500/5 transition-all group"
                >
  ...
  return (
    <div className="max-w-6xl mx-auto h-[calc(100vh-140px)] flex flex-col gap-6 animate-fade-in relative">
      {/* HUD: Combat Metrics */}
      <div className="flex justify-between items-center bg-white/[0.02] border border-white/5 rounded-2xl p-6 backdrop-blur-xl">
        <div className="flex items-center gap-6">
           <div>
             ...
             <div className="text-white font-bold text-lg tracking-tight">Logical Defenses</div>
           </div>
        ...
        <div className="text-right">
           ...
           <div className="serif italic text-xl text-white">"The Sophist"</div>
        </div>
      </div>
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto space-y-8 p-6 surface rounded-2xl mb-4 scrollbar-thin scrollbar-thumb-white/5 no-scrollbar"
          >
            {session.history.map(turn => (
              <div key={turn.id} className={`flex gap-4 ${turn.role === 'user' ? 'flex-row-reverse' : 'flex-row'} animate-in slide-in-from-bottom-2`}>
                ...
                <div className={`max-w-[80%] p-6 rounded-2xl relative ${
                  turn.role === 'user' 
                    ? 'bg-white/[0.03] border border-white/5 text-white italic font-light serif text-lg' 
                    : 'bg-rose-950/10 border border-rose-500/20 text-rose-50 font-mono text-sm leading-relaxed'
                }`}>
                  {turn.content}
                  
                  {turn.calloutDetected && (
                    <div className={`mt-4 p-4 rounded-xl border text-[11px] font-sans ${
          ...
          <div className="relative">
            <input 
              ...
              placeholder="Counter the sophistry..."
              disabled={loading || session.status !== 'active'}
              className="w-full bg-white/[0.02] border border-white/10 rounded-2xl py-6 pl-8 pr-20 text-white focus:ring-1 focus:ring-rose-500/50 outline-none transition-all placeholder:text-zinc-800 serif italic text-lg"
            />
            <button 
              onClick={handleSend}
              disabled={!input.trim() || loading || session.status !== 'active'}
              className="absolute right-4 top-1/2 -translate-y-1/2 p-4 bg-rose-600 text-white rounded-xl hover:bg-rose-700 transition-all shadow-2xl disabled:opacity-0"
            >
              <Send size={18} />
            </button>
          </div>
        ...
        <aside className="w-80 flex flex-col gap-6">
          <div className="surface p-6 rounded-2xl flex-1 flex flex-col">
             <div className="mb-6">
                ...
                <p className="text-[10px] text-slate-600 uppercase tracking-widest">Identify the injected fallacy.</p>
             </div>
             ...
             <div className="flex-1 overflow-y-auto space-y-2 no-scrollbar">
                {FALLACIES.map(fallacy => (
                  <button
                    ...
                    disabled={loading || session.status !== 'active'}
                    className="w-full text-left p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-indigo-500/40 hover:bg-indigo-500/5 transition-all group"
                  >
                    <div className="text-[11px] text-white font-bold group-hover:text-indigo-400 transition-colors">{fallacy.name}</div>
                  </button>
                ))}
             </div>
          </div>
          ...
          <button 
            ...
            className="p-4 rounded-xl border border-white/5 bg-white/[0.02] text-[10px] uppercase tracking-[0.4em] text-slate-500 hover:text-white hover:bg-rose-600/10 hover:border-rose-600/20 transition-all flex items-center justify-center gap-3"
          >
