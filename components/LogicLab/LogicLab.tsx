...
import { Fallacy, AppState } from '../../types';
import { 
  Loader2, 
  FlaskConical, 
  ChevronRight, 
...
export const LogicLab: React.FC<LogicLabProps> = ({ state, updateProgress }) => {
  ...
  const startLab = async () => {
    ...
    // Pick a fallacy with low mastery or random
  ...
  const handleIdentify = () => {
    if (guess === targetFallacy?.id) {
      setPhase('repair');
    } else {
      setError("Not quite. Take another look at the logic.");
    }
  };
  ...
  const handleRepairSubmit = async () => {
    ...
    setError(null);
    try {
      const result = await evaluateRepair(statement, targetFallacy!.name, userRepair);
      setEvaluation(result);
      setPhase('result');
      
      // Update progress based on score
      const quality = Math.ceil(result.score / 20); // Scale 0-100 to 0-5
      ...
      {phase === 'identify' && (
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          <div className="surface p-8 rounded-2xl border border-zinc-800 relative overflow-hidden">
             <MessageSquareQuote className="absolute top-4 right-4 text-zinc-800 w-12 h-12" />
             <h3 className="text-xs font-bold text-rose-500 uppercase tracking-widest mb-4">Fallacious Statement</h3>
             <p className="text-2xl font-serif text-white italic leading-relaxed">"{statement}"</p>
          </div>

          <div className="space-y-4">
             <label className="text-sm font-medium text-slate-400">Identify the primary fallacy:</label>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
               {[...FALLACIES].sort(() => 0.5 - Math.random()).slice(0, 4).concat(targetFallacy!).sort((a,b) => a.name.localeCompare(b.name)).map(f => (
                 <button
                   key={f.id}
                   onClick={() => { setGuess(f.id); setError(null); }}
                   className={`p-4 text-left rounded-xl border transition-all ${
                     guess === f.id ? 'bg-rose-500/10 border-rose-500 text-white' : 'bg-zinc-900 border-zinc-800 text-slate-400 hover:border-zinc-700'
                   }`}
                 >
                   <p className="font-bold text-sm">{f.name}</p>
                   <p className="text-[10px] text-slate-500 mt-1 line-clamp-1">{f.definition}</p>
                 </button>
               ))}
             </div>
          </div>
          ...
          <div className="flex justify-end pt-4">
             <button 
               ...
               disabled={!guess}
               className="btn-primary bg-rose-600 hover:bg-rose-700 px-6 py-2 rounded-lg flex items-center gap-2 disabled:opacity-50"
             >
               Confirm Identification <ChevronRight size={18} />
             </button>
          </div>
        </div>
      )}
      ...
      {phase === 'repair' && (
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
           <div className="bg-emerald-900/10 border border-emerald-800/30 p-4 rounded-xl flex items-start gap-3">
              ...
              <div>
                <p className="text-emerald-200 text-sm font-bold">Correct! That is a {targetFallacy?.name}.</p>
                <p className="text-emerald-500/70 text-xs mt-1">{targetFallacy?.definition}</p>
              </div>
           </div>

           <div className="space-y-4">
              <label className="text-sm font-medium text-slate-400">Your Challenge: Repair the Argument</label>
              <div className="surface p-8 rounded-2xl border border-zinc-800 bg-zinc-900/30">
                 <p className="text-slate-500 text-xs uppercase font-bold tracking-widest mb-2">Original</p>
                 <p className="text-slate-300 italic mb-6">"{statement}"</p>
                 
                 <p className="text-slate-500 text-xs uppercase font-bold tracking-widest mb-2">Steel Man Version</p>
                 <textarea 
              ...
              <div className="space-y-4">
                 <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                       ...
                       <p className="text-slate-400 text-sm italic">"{statement}"</p>
                    </div>
                 </div>
